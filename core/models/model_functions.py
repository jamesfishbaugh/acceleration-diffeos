...
def initialize_impulse(intial_impulse, number_of_time_points, number_of_control_points, dimension):

  if intial_impulse is not None:
    print('>> NOT YET IMPLEMENTED, INITIALIZING AS 0')
    
  impulse_t = np.zeros((number_of_time_points, number_of_control_points, dimension))
    
  return impulse_t

def initialize_initial_velocity(initial_velocity, number_of_control_points, dimension):
    if initial_velocity is not None:
        velocity = read_3D_array(initial_velocity)
        print('>> Reading initial velocity from file: %s.' % initial_velocity)

    else:
        velocity = np.zeros((number_of_control_points, dimension))
        #velocity = np.random.rand(number_of_control_points, dimension)
