...
def compute_noise_dimension(template, multi_object_attachment, dimension, objects_name=None):
    ...
    for k in range(len(template.object_list)):
        ...
        elif multi_object_attachment.attachment_types[k] in ['L2', 'ncc']:
...
def _get_norm_for_object(object, object_id):
    ...
    if object_type == 'SurfaceMesh'.lower() or object_type == 'PolyLine'.lower():
        ...
        if 'attachment_type' in object.keys() and object['attachment_type'].lower() == 'L2'.lower():

            object_norm = 'L2'.lower()
        elif 'attachment_type' in object.keys() and object['attachment_type'].lower() == 'NCC'.lower():

            object_norm = 'NCC'.lower()
        else:
            msg = 'Defaulting to L2'
            warnings.warn(msg)
            object_norm = 'L2'.lower()
